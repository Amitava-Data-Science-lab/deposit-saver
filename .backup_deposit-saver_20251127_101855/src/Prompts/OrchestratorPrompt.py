...
- state_persister_tool(state_key, state_value)
...
- CRITICAL PERSISTENCE STEP: Immediately after validating the successful output of any specialist agent 
 (HousingGoalAgent, BankDataAgent, RiskProfilerAgent, PlanGeneratorAgent), you MUST call the 
 state_persister_tool with the validated output object and the corresponding state key 
(e.g., 'housing_goal', 'risk_profile'). This ensures the critical data is saved persistently.
- **You should use the validated output from the tool call to populate the corresponding state (e.g., 'housing_goal', 'saving_capacity') before proceeding to the next conversational turn.**
